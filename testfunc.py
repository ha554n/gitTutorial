...
#%% subject class
    class Subject():
        ...
        def __init__(self,df,subname):
            ...
            #for 0 diff, makes it positive, so do not use 'sign' for getting strings unless
            #correcting somehow by equalize
            ...
            def getPreLengths(countsdf,npar,kind):
                '''
                ...
                pre=np.where(countsdf[self.subname+'PRE']>=countsdf[self.subname+'POST'])[0]#indices where pre>post
            ...
            def getPostLengths(countsdf,npar,kind):
                #IF >= MEANS COUNTS TIES (0) FOR BOTH STRINGS!
                if kind=='sign':
                post=np.where(countsdf.Sign>0)[0]
                if kind=='diff':
                post=np.where(countsdf[self.subname+'PRE']<=countsdf[self.subname+'POST'])[0]
            ...
            def getTrueCountsInfo(countsdf,npar):
                ...
                """
                [preIdx,preLen]=getPreLengths(countsdf,npar,'diff')#get pre strings
                [postIdx,postLen]=getPostLengths(countsdf,npar,'diff')#get post strings
     ...
     def runBTsim(subname,itera):
        ...
        def getPreLengths(countsdf,npar,kind):
            ...
            if kind=='sign':
              ...
              pre=np.where(countsdf['PRE']>=countsdf['POST'])[0]#indices where pre=>post
            addCircle=all(x in pre for x in [npar-1,0])#if any of the strings are at edges then have to make circular
            prelist=[]
            for k,g in groupby(enumerate(pre),lambda ix:ix[0]-ix[1]):
                #this returns consecutive numbers. ex. if strings are [2,3,4,6,7,8,11,12]
        ...
        def getPostLengths(countsdf,npar,kind):
            ...
            if kind=='sign':
              ...
              post=np.where(countsdf['POST']>=countsdf['PRE'])[0]
            addCircle=all(x in post for x in [npar-1,0])
            postlist=[]
            for k,g in groupby(enumerate(post),lambda ix:ix[0]-ix[1]):
                postlist.append(list(map(itemgetter(1),g)))
        ...
        def simulateBT_ha(idx,stims,npar,dfprobs):
            ...
            AllCompetition=pd.DataFrame(np.sum(pre_CompetitionMat_df),columns=['PRE'])#add random noise to break ties
            AllCompetition['POST']=np.sum(post_CompetitionMat_df)
            AllCompetition['DIFF']=np.round(AllCompetition.POST-AllCompetition.PRE)
        ...
        subs[subname].Shirts_sim=pd.DataFrame()
        start=time.process_time()
        
        for i in range(itera):
            subs[subname].Shirts_sim=subs[subname].Shirts_sim.append(simulateBT_ha(subs[subname].shirts,subs[subname].ShirtsStims,16,subs[subname].Shirts_dfprobs))
        ...
        subs[subname].Cars_sim=pd.DataFrame()
        start=time.process_time()
        
        for i in range(itera):
            subs[subname].Cars_sim=subs[subname].Cars_sim.append(simulateBT_ha(subs[subname].cars,subs[subname].CarsStims,16,subs[subname].Cars_dfprobs))
        ...
        subs[subname].Faces_sim=pd.DataFrame()
        start=time.process_time()
        
        for i in range(itera):
            subs[subname].Faces_sim=subs[subname].Faces_sim.append(simulateBT_ha(subs[subname].faces,subs[subname].FacesStims,16,subs[subname].Faces_dfprobs))
